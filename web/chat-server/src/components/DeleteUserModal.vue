<template>
  <div class="manager-table-pane" v-if="isVisible">
    ...
    <div class="manager-table-bar">
      <span class="manager-table-bar__count">已选 {{ uuidList.length }} 项,删除后不可恢复</span>
      <el-button class="soft-action-btn soft-action-btn--danger" @click="deleteUsers">删除选中</el-button>
    </div>
  </div>
</template>
    ...
    const deleteUsers = async () => {
      ...
      try {
        await ElMessageBox.confirm(
          `确定删除选中的 ${data.uuidList.length} 个用户?删除后不可恢复。`,
          "删除用户",
      ...
      try {
        ...
        await axios.post(store.state.apiUrl + "/admin/deleteUsers", req);
